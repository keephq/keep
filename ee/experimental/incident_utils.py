...
def mine_incidents(
    ...
    mined_incidents = shape_incidents(
        alert_df,
        "fingerprint",
    ...
    return [
        {
            ...
            "alerts": [
                alert
                for alert in alerts
                if alert.fingerprint in incident["alert_fingerprints"]
            ],
        }
...
def get_batched_alert_counts(
    ...
    resampled_alert_counts = (
        ...
        .resample(f"{sliding_window_size//2}s")[unique_alert_identifier]
        .value_counts()
...
def build_graph_from_occurrence(
    occurrence_row: pd.DataFrame,
    jaccard_matrix: np.ndarray,
    unique_alert_identifiers: List[str],
...
def shape_incidents(
    ...
    incident_sliding_window_size: int,
    statistic_sliding_window_size: int,
    jaccard_threshold: float = 0.2,
    fingerprint_threshold: int = 5,
) -> List[dict]:
    """
    ...
    resampled_alert_counts = (
        alerts.set_index("timestamp")
        .resample(f"{incident_sliding_window_size//2}s")[unique_alert_identifier]
        .value_counts()
        .unstack(fill_value=0)
    )
    ...
    for idx in range(resampled_alert_counts.shape[0]):
        graph = build_graph_from_occurrence(
            resampled_alert_counts.iloc[idx],
            jaccard_matrix,
            resampled_alert_counts.columns,
            jaccard_threshold=jaccard_threshold,
        )
        max_component = max(nx.connected_components(graph), key=len)

        min_starts_at = resampled_alert_counts.index[idx]
        max_starts_at = min_starts_at + pd.Timedelta(
            seconds=incident_sliding_window_size
        )

        local_alerts = alerts[
            (alerts["timestamp"] >= min_starts_at)
            & (alerts["timestamp"] <= max_starts_at)
        ]
        local_alerts = local_alerts[
            local_alerts[unique_alert_identifier].isin(max_component)
        ]
            ...
            incidents.append(
                {
                    ...
                    "alert_fingerprints": local_alerts[unique_alert_identifier]
                    .unique()
                    .tolist(),
                }
            )
