...
def get_last_incidents_by_cel(
    ...
    with Session(engine) as session:
        ...
        sql = f"""
                ...
                WHERE
                    incident.tenant_id = 'keep'
                    {f'AND {query_metadata.where}' if query_metadata.where else ''}
                GROUP BY 
                    incident.id
            """
        ...
        all = session.exec(text(sql)).all()
        incidents = [Incident(**row._asdict()) for row in all]
