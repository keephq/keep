...
def _generate_rss_feed(alerts: list[AlertDto], preset_name: str, base_url: str = "") -> str:
    """
    Generate RSS XML feed from alerts.
    
    Args:
        alerts: List of AlertDto objects
        preset_name: Name of the preset for the feed title
    ...
    rss_content.append(f'<title>Keep Alerts - {escape(preset_name)}</title>')
    rss_content.append(f'<description>Alert feed for {escape(preset_name)} preset</description>')
    rss_content.append(f'<link>{escape(base_url)}</link>')
    rss_content.append('<language>en-us</language>')
    rss_content.append(f'<lastBuildDate>{datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S GMT")}</lastBuildDate>')
    rss_content.append('<generator>Keep Alert Management Platform</generator>')
    
    # Add items for each alert
    for alert in alerts:
        rss_content.append('<item>')
        
        # Title: alert name with severity and status
        title = f"[{alert.severity.value.upper() if alert.severity else 'UNKNOWN'}] {alert.name}"
        if alert.status:
            title += f" ({alert.status.value.upper()})"
        rss_content.append(f'<title>{escape(title)}</title>')
        
        # Description: combine description, environment, and source info
        description_parts = []
        if alert.description:
            description_parts.append(alert.description)
        if alert.environment:
            description_parts.append(f"Environment: {alert.environment}")
        if alert.source:
            description_parts.append(f"Source: {', '.join(alert.source)}")
        if alert.service:
            description_parts.append(f"Service: {alert.service}")
        
        description = " | ".join(description_parts) if description_parts else "No description available"
        rss_content.append(f'<description>{escape(description)}</description>')
        
        # Link: use alert URL if available, otherwise link to alert feed
        link = alert.url if alert.url else f"{base_url}/alerts/feed"
        rss_content.append(f'<link>{escape(str(link))}</link>')
        
        # GUID: use fingerprint as unique identifier
        rss_content.append(f'<guid isPermaLink="false">{escape(alert.fingerprint or alert.id or "")}</guid>')
        
        # Publication date: use lastReceived
        try:
            pub_date = datetime.fromisoformat(alert.lastReceived.replace('Z', '+00:00')).strftime("%a, %d %b %Y %H:%M:%S GMT")
        except:
            pub_date = datetime.utcnow().strftime("%a, %d %b %Y %H:%M:%S GMT")
        rss_content.append(f'<pubDate>{pub_date}</pubDate>')
        
        # Category: use severity as category
        if alert.severity:
            rss_content.append(f'<category>{escape(alert.severity.value)}</category>')
...
@router.get(
    "/{preset_name}/rss",
    description="Get RSS feed for preset alerts",
    response_class=FastAPIResponse,
)
def get_preset_rss_feed(
    request: Request,
    bg_tasks: BackgroundTasks,
    preset_name: str,
    authenticated_entity: AuthenticatedEntity = Depends(
    ...
    """
    Generate an RSS feed for alerts from a specific preset.
    This endpoint provides a very basic RSS feed implementation for alerts.
    """
    # Gathering alerts may take a while and we don't care if it will finish before we return the response.
    # In the worst case, gathered alerts will be pulled in the next request.
    bg_tasks.add_task(
    ...
    # Generate RSS feed
    rss_content = _generate_rss_feed(preset_alerts, preset_name, base_url)
    
    logger.info(
        "Generated RSS feed for preset", 
        extra={"preset_name": preset_name, "alert_count": len(preset_alerts)}
    )
