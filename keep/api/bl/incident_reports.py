...
#     incidents_in_january_2025: Optional[int] = None
#     incidents_in_february_2025: Optional[int] = None
#     peak_incident_date: Optional[str] = None
#     peak_incident_count: Optional[int] = None
...
class IncidentReport(BaseModel):
    ...
    most_incident_reasons: Optional[list[str]] = None
...
1. **Mean Time to Detect (MTTD)**
   - Compute `mean_time_to_detect` as the average difference (in milliseconds) between `start_time` and `creation_time`.

2. **Mean Time to Resolve (MTTR)**
   - Compute `mean_time_to_resolve` as the average difference (in milliseconds) between `resolution_time` and `start_time`.
...
4. **Incident Duration Metrics**
   - Identify the shortest and longest incidents based on duration (`resolution_time - start_time` in milliseconds).
   - Output:
     - `shortest_duration_ms`: Duration of the shortest incident.
     - `shortest_duration_incident_id`: ID of the shortest incident.
     - `longest_duration_ms`: Duration of the longest incident.
     - `longest_duration_incident_id`: ID of the longest incident.
...
7. **Most Frequent Incident Reasons**
   ...
   - Output `most_frequent_reasons` as a dictionary with reasons as keys and count as values.
"""
...
class IncidentReports:
    ...
    def get_incident_reports(
        ...
        #     [item.dict() for item in query_result.items], default=str
        # )
        incidents_json = incidents_json_hardcoded
        # foo = IncidentReport.schema()
        ...
        #             "description": "Generates an incident report with key metrics.",
        #             "parameters": IncidentReport.schema(),  # Pydantic schema converted to JSON Schema
        #         },
        #     }
        # ]

        # schema = IncidentReport.schema_json()
        ...
        return IncidentReport(**json.loads(foster))
