...
async def process_correlation(ctx, tenant_id: str):
    ...
    await mine_incidents_and_create_objects(ctx, tenant_id=tenant_id)
...
async def process_summary_generation(ctx, tenant_id: str, incident_id: str):
    ...
    await generate_update_incident_summary(
        ctx, tenant_id=tenant_id, incident_id=incident_id
    )
    end_time = datetime.datetime.now()
    logger.info(
...
async def process_background_ai_task(
    ...
    try:
        ...
        logger.error(
            f"Error getting queued jobs, happens sometimes with unknown reason: {e}"
        )
        return None
    ...
    if mine_incidents_and_create_objects is not NotImplemented:
        ...
        for tenant in tenants:
            if is_ee_enabled_for_tenant(
                tenant, tenant_configuration=tenant_configuration
            ):
                ...
                jobs_with_same_prefix = [
                    job for job in all_jobs if job.job_id.startswith(job_prefix)
                ]
                if len(jobs_with_same_prefix) > 0:
                    ...
                    job = await pool.enqueue_job(
                        ...
                        _job_id=job_prefix
                        + ":"
                        + str(time.time()),  # Strict ID ensures uniqueness
                        _job_try=1,
                    )
                    logger.info(
