...
Create Date: 2025-05-02 15:40:00.000000
...
down_revision = "885ff6b12fed"  # Same as the original parent
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    if 'commentmention' not in inspector.get_table_names():
        ...
        # Create indexes
        op.create_index(
            "ix_comment_mention_comment_id", "commentmention", ["comment_id"], unique=False
        )
        op.create_index(
        ...
        op.create_index(
            "ix_comment_mention_tenant_id", "commentmention", ["tenant_id"], unique=False
        )
