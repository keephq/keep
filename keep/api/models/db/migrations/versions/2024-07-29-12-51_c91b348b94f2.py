...
def depopulate_db(session):
    ...
    session.execute(sa.update(incident_table).values(description=incident_table.c.user_summary))
    session.commit()
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column("incident", sa.Column("description", sa.VARCHAR(), nullable=False, default="", server_default=""))
