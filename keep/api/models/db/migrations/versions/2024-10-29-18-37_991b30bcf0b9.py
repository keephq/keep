...
Fixing the value in this migration.""")
        ...
        result = connection.execute(
            sa.text(
                "SELECT incident_id, alert_id, deleted_at FROM alerttoincident WHERE deleted_at = '1000-01-01 00:00:00'"
            )
        )
        ...
        for row in result:
            try:
                ...
                print(
                    f"Updated deleted_at for incident_id: {row['incident_id']}, alert_id: {row['alert_id']}"
                )
            except sa.exc.IntegrityError as e:
                if (
                    "UNIQUE constraint failed: alerttoincident.alert_id, alerttoincident.incident_id, alerttoincident.deleted_at"
                    in str(e)
                ):
                    connection.execute(
                        sa.text(
                    ...
                    logger.warning(
                        f"IntegrityError encountered for incident_id: {row['incident_id']}, alert_id: {row['alert_id']}. It's a duplicate. Deleted."
                    )
                else:
                    raise
    else:
