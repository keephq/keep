...
def upgrade() -> None:
    ...
    workflows = connection.execute(
        sa.text("SELECT id, workflow_raw FROM workflow")
    ).fetchall()
    ...
    for workflow in workflows:
        try:
            ...
            if Parser.parse_disabled(workflow_yaml):
                updates.append({"id": workflow.id, "is_disabled": True})
    ...
    if updates:
        connection.execute(
            sa.text("UPDATE workflow SET is_disabled = :is_disabled WHERE id = :id"),
            updates,
        )
...
def downgrade() -> None:
    ...
    for workflow in workflows:
        try:
            ...
            if "disabled" in workflow_yaml:
                ...
                updates.append(
                    {"id": workflow.id, "workflow_raw": updated_workflow_raw}
                )
        except Exception as e:
            print(f"Failed to parse workflow_raw for workflow id {workflow.id}: {e}")
            continue
    ...
    if updates:
        connection.execute(
            sa.text("UPDATE workflow SET workflow_raw = :workflow_raw WHERE id = :id"),
            updates,
        )
