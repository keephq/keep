...
from keep.api.core.db import assign_alert_to_group as assign_alert_to_group_db, assign_alert_to_incident, create_incident_from_dict
...
class RulesEngine:
    ...
    def run_rules(self, events: list[AlertDto]) -> list[IncidentDto] | None:
        ...
        grouped_incidents: list[Incident] = []
        for group in groups:
            ...
            # insert "keep" as the first source to emphasize that this alert was generated by keep
            ...
            incident_data = {
                ...
                "last_received": group_attributes.get("last_update_time"),
                "description": group_description,
                "is_predicted": True,
            }
            ...
            for alert in group.alerts:
                assign_alert_to_incident(alert.id, incident_id, self.tenant_id)

            # Append the incident to the list
            grouped_incidents.append(incident)

            self.logger.info(f"Created incident {incident_id} for group {group.id}")
        self.logger.info(f"Rules ran, {len(grouped_incidents)} incidents created")
        incidents_dto = [IncidentDto.from_db_incident(incident) for incident in grouped_incidents]
