...
class Postgres2ProviderAuthConfig:
    host: str = dataclasses.field(
        metadata={"required": True, "description": "PostgreSQL Host"},
        default=""
    )
    port: int = dataclasses.field(
        metadata={"description": "PostgreSQL Port"},
        default=5432
    )
...
class Postgres2Provider(BaseProvider):
    """PostgreSQL database provider."""
    ...
    def validate_config(self):
        self.authentication_config = Postgres2ProviderAuthConfig(**self.config.authentication)
    ...
    def _notify(self, query: str = "", **kwargs: Dict[str, Any]):
        ...
        if not HAS_POSTGRES:
            raise ProviderException("psycopg2 is not installed")
        ...
        try:
            ...
            raise ProviderException(f"PostgreSQL error: {e}")

        self.logger.info("PostgreSQL query executed")
