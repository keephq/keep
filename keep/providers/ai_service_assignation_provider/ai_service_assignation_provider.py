...
class AiServiceAssignationProvider(BaseAiProvider):
    ...
    def _query(self, services_and_descriptions, **kwargs):
        ...
        prompt = "; ".join(
            [
                f'Service: "{service.service}" ({service.description})'
                for service in services_and_descriptions
            ]
        )
        prompt += " Assign only one service, choose between: "
        prompt += ", ".join([service.service for service in services_and_descriptions])

        template = {
            "service": "|".join(
                [
                    str(k)
                    for k in [service.service for service in services_and_descriptions]
                ]
            ),
        }
        alert = self.context_manager.get_full_context()["alert"]
        enrichment = self._execute_model(
            template=template, instruction=prompt, alert=alert.name, as_enrichment=True
        )
