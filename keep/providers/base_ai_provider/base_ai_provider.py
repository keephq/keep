    ...
    def _execute_model(self, template, instruction, alert, as_enrichment=False):
        ...
        with OllamaLock.lock:
            ...
            try:
                ...
                ollama.Client(host=self.authentication_config.host).pull(model_name, host=self.authentication_config.host)
            except ConnectError:
                self.logger.error(f"Failed to connect to Ollama. Please check Ollama host.")
                return {}

            self.logger.info(f"Executing model: {model_name} with instruction: {instruction} and alert {alert}.")
        ...
        try:
            ...
            if as_enrichment:
                if len(json_response) != 1:
                    self.logger.info(f"Alert could be enriched with only one field, but: {json_response} was given.")
                    return {}
