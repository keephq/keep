try {
  ...
  // Nagios-specific incident key using macros
  var incidentKey = "nagios-" + (params["HOST_EXECUTION_START"] || Date.now());

  req.addHeader("Accept: application/json");
  req.addHeader("Content-Type: application/json");
  req.addHeader("X-API-KEY: " + keepApiKey);

  Zabbix.log(4, "[Keep Webhook] keepApiUrl:" + keepApiUrl);  // Legacy log, adapt if Nagios logging differs
