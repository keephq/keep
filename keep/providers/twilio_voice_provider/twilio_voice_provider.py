"""Twilio Voice provider for phone calls."""
...
class TwilioVoiceProviderAuthConfig:
    account_sid: str = dataclasses.field(
        metadata={"required": True, "description": "Twilio Account SID"},
        default=""
    )
...
class TwilioVoiceProvider(BaseProvider):
    ...
    PROVIDER_DISPLAY_NAME = "Twilio Voice"
    ...
    def validate_config(self):
        self.authentication_config = TwilioVoiceProviderAuthConfig(**self.config.authentication)
    ...
    def _notify(self, to: str = "", message: str = "", **kwargs: Dict[str, Any]):
        ...
        url = f"{self.TWILIO_API}/{self.authentication_config.account_sid}/Calls.json"
        ...
        try:
            ...
            raise ProviderException(f"Twilio Voice API error: {e}")

        self.logger.info("Phone call initiated via Twilio")
