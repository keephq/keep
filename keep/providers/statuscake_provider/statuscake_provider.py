...
class StatuscakeProvider(BaseProvider):
    ...
    def __get_heartbeat_alerts(self) -> list[AlertDto]:
        try:
            ...
            if not response.ok:
                self.logger.error(
                    "Failed to get heartbeat from Statuscake: %s", response.json()
                )
    ...
    def __get_pagespeed_alerts(self) -> list[AlertDto]:
        try:
            response = requests.get(
                "https://api.statuscake.com/v1/pagespeed/",
                headers=self.__get_auth_headers(),
            )

            if not response.ok:
                self.logger.error(
                    "Failed to get pagespeed from Statuscake: %s", response.json()
                )
    ...
    def __get_ssl_alerts(self) -> list[AlertDto]:
        try:
            response = requests.get(
                "https://api.statuscake.com/v1/ssl/", headers=self.__get_auth_headers()
            )

            if not response.ok:
                self.logger.error(
                    "Failed to get ssl from Statuscake: %s", response.json()
                )
    ...
    def __get_uptime_alerts(self) -> list[AlertDto]:
        try:
            ...
            if not response.ok:
                self.logger.error(
                    "Failed to get uptime from Statuscake: %s", response.json()
                )
