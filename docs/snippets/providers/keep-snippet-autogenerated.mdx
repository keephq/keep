...
actions:
    - name: Query keep
      ...
      with:
        ...
        if_condition: {value}  # condition to evaluate for alert creation
        for_duration: {value}  # duration for state alerts
        if: {value}  # 'if' parameter is deprecated. Use `if_condition` instead.
        for: {value}  # 'for' parameter is deprecated. Use `for_duration` instead.
```
