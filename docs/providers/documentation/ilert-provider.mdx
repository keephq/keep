...
The ilert provider enables the creation, updating, and resolution of events or incidents on ilert, leveraging both incident management and event notification capabilities for effective incident response. This provider integrates Keep's system with ilert's robust alerting and incident management platform.

## Inputs

The `_type` parameter specifies the nature of the notification or action to be taken via the ilert API:

- **incident**: Used for creating or updating incidents. Requires details such as incident summary, status, message, and affected services.
- **event**: Allows sending customized event notifications, configured to alert, accept, or resolve specific conditions. Supports details like event type, summary, custom details, and links for more context.

The provider routes the operation to the appropriate endpoint based on the specified `_type` and handles the data according to ilert's requirements for incidents or events.

### Incident Management

- `summary`: A brief summary of the incident. Required for creating a new incident.
