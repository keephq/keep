...
Keep's Maintenance Windows feature provides a critical mechanism for managing alert noise during scheduled maintenance periods or other planned events. By defining Maintenance Window rules, users can suppress alerts (**meaning those alerts will be dropped out by Keep and not shown in alerts feed**) that are irrelevant during these times, ensuring that only actionable alerts reach the operations team.
