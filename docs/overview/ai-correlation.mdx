...
The AI correlation engine can be configured to optimize for different goals:

-   **Quality**: Prioritizes higher accuracy in correlating alerts and incidents. This might involve longer training times and higher correlation thresholds.
-   **Speed**: Prioritizes faster model training and inference. This might result in slightly lower accuracy but quicker processing of alerts.
-   **Resource**: Prioritizes minimizing computational resources (e.g., CPU, memory). This might lead to trade-offs in both speed and quality but is ideal for environments with limited resources.
