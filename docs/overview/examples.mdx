...
<<<<<<< HEAD
2. Create a workflow that is triggered by the alert, runs an SQL query, and decides whether to create an incident.  Once the workflow is created, you can upload it via the [Workflows](workflow) page.
=======
...
<<<<<<< HEAD
2. Create a straightforward Workflow that filters for CloudWatch events and sends a Slack message:
=======
