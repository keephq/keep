...
<Info>
  Keep uses Server-Sent Events (SSE) for real-time updates (alerts, incidents,
  presets, topology). The SSE endpoint is served by the backend at `/sse/events`.
  No separate WebSocket server is required. When **AUTH_TYPE** is noauth, the
  SSE endpoint accepts unauthenticated connections and uses the default tenant.
</Info>

|        Env var        |              Purpose              | Required | Default Value |   Valid options   |
| :-------------------: | :-------------------------------: | :------: | :-----------: | :---------------: |
|  **PUSHER_DISABLED**  | Disables real-time SSE updates    |    No    |    "false"    | "true" or "false" |
...
### Real-time (SSE) – frontend

<Info>
  The frontend uses Server-Sent Events (SSE) for real-time updates. The backend
  serves the SSE stream at `/sse/events`. Set **PUSHER_DISABLED** to "true" to
  disable real-time updates in the UI.
</Info>
