...
- The `get-customer-details` step fetches customer data based on the alert.
- The `enrich_alert` directive adds `customer_name` and `customer_tier` to the alert.
- The `customer_name` enrichment is marked as disposable, meaning it will be discarded when a new alert is received.
---
## Enrichment Syntax
### Key-Value Pairs
Each enrichment is defined as a key-value pair:
...
If there is no alert/incident present in the trigger (for example interval trigger or manual call in workflow page), the enrichment rule would not have an alert/incident to apply to. The enrichment process typically requires an alert/incident to be present to apply the specified enrichments. Without an alert/incident, the enrichment rule would not execute as intended.

### Calling subprocesses
Regarding calling subprocesses within a trigger interval and passing variables, workflow not provide this capability. However, workflow allows for manual triggers and interval-based triggers, which can be configured to perform actions like polling and enriching alerts. You might need to explore custom implementations or consult further documentation for subprocess handling and variable passing.

Some possible workarounds for calling subprocesses within a trigger interval include:

* Using a separate workflow for the subprocess and triggering it manually or through an API call.
* Consulting with Keep's support team or community forums for more information on custom implementations and best practices.
