...
Runs workflows when a maintenance window is created, updated, or deleted. This is useful for notifying stakeholders when scheduled maintenance changes.
...
The maintenance window is available to the workflow through the `maintenance` context, for example `{{ maintenance.name }}`, `{{ maintenance.start_time }}` and `{{ maintenance.end_time }}`.
