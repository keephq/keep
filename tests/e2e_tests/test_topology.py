...
def test_topology_manual(browser):
    try:
        ...
        expect(browser.get_by_role("button", name="Save", exact=True)).not_to_be_disabled()
        browser.get_by_role("button", name="Save", exact=True).click()
        browser.wait_for_timeout(1000)
        
        # Validate that the node was added
        node_with_text_1 = browser.locator("div.react-flow__node").filter(has_text="SERVICE_ID_1")
        ...
        edge_1_to_2 = browser.locator("g.react-flow__edge[aria-label='Edge from 1 to 2']")
        expect(edge_1_to_2).to_have_count(1)
        edge_1_to_3 = browser.locator("g.react-flow__edge[aria-label='Edge from 1 to 3']")
        ...
        expect(browser.locator("g.react-flow__edge[aria-label='Edge from 1 to 2']")).to_have_count(0)
        
        # Ensure remaining edges are intact
        expect(browser.locator("g.react-flow__edge[aria-label='Edge from 1 to 3']")).to_have_count(1)
        browser.wait_for_timeout(2000)
        
        # Delete a node and ensure related edges are removed
        node_to_delete = browser.locator("div.react-flow__node").filter(has_text="SERVICE_ID_1")
        node_to_delete.click()
        browser.wait_for_timeout(2000)
        browser.get_by_role("button", name="Delete Service", exact=True).click()
        browser.wait_for_timeout(1000)
        expect(browser.locator("div.react-flow__node").filter(has_text="SERVICE_ID_1")).to_have_count(0)
        expect(browser.locator("g.react-flow__edge[aria-label='Edge from 1 to 3']")).to_have_count(0)
        
        # Update node name and verify the change
        node_to_update = browser.locator("div.react-flow__node").filter(has_text="SERVICE_ID_2")
        ...
        browser.wait_for_timeout(3000)
        expect(browser.locator("div.react-flow__node").filter(has_text="UPDATED_SERVICE")).to_have_count(1)
