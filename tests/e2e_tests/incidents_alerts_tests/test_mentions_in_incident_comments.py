...
def test_mentions_in_incident_comments(
    ...
    try:
        ...
        page.locator("div").filter(has_text=re.compile(r"^Summary$")).get_by_role(
            "paragraph"
        ).nth(1).click()
        page.locator("div").filter(has_text=re.compile(r"^Summary$")).locator(
            "div"
        ).nth(3).fill("Test summary")
