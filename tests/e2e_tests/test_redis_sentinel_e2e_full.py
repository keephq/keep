...
class SentinelE2ETest:
    ...
    def run_command(
        self, cmd: list[str], timeout: int = 60, capture_output: bool = True
    ) -> tuple[int, str, str]:
    ...
    def start_infrastructure(self):
        ...
        returncode, _stdout, stderr = self.run_command(
            cmd, timeout=300
        )  # 5 minutes for build
        if returncode != 0:
    ...
    def wait_for_keep_api_healthy(self) -> bool:
        ...
        logger.error(
            f"Keep API did not become healthy within {HEALTH_CHECK_TIMEOUT} seconds"
        )
        return False
    ...
    def check_redis_keys(self) -> bool:
        ...
        cmd = [
            ...
            "--rm",
            "--network",
            f"{self.project_name}_keep-test",
            "redis:7-alpine",
            "redis-cli",
            "-h",
            "redis-master",
