...
function CustomSelect({ filter, setLocalFilter }: { filter: Filter | null, setLocalFilter: (value: string | string[]) => void }) {
    ...
    return (
        <>
            <span className="text-gray-400 text-sm">Select {filterKey?.charAt(0)?.toUpperCase() + filterKey?.slice(1)}</span>
...
function CustomDate({ filter, handleDate }: { filter: Filter | null, handleDate: (from?: Date, to?: Date) => void }) {
    ...
    return <div className="flex justify-center items-center m-x-4">
        <DateRangePicker
            value={dateRange}
...
const PopoverContent: React.FC<PopoverContentProps> = ({ filterRef, filterKey, type }) => {
    ...
    const handleDate = (start?: Date, end?: Date) => {
        ...
        if (!start && !end) {
            ...
            newValue = JSON.stringify({
                start: start,
                end: moment(end || start).endOf('day').toDate(),
            })
        }
        if (filter) {
    ...
    switch (type) {
        case "select": return <CustomSelect filter={localFilter} setLocalFilter={handleLocalFilter} />
        case "date": return <CustomDate filter={localFilter} handleDate={handleDate} />
        default: return null;
    }
};
...
export const GenericFilters: React.FC<FiltersProps> = ({ filters }) => {
    ...
    useEffect(() => {
        if (apply && filterRef.current) {
            console.log("filterRef.current in filters", filterRef.current);
    ...
    return (
        <div className="relative flex flex-col md:flex-row lg:flex-row gap-4 items-center">
            {filters && filters?.map(({ key, type, name }) => {
                return <div key={key} className="flex gap-4">
                    <GenericPopover
                        triggerText={name}
                        triggerIcon={GoPlusCircle}
                        content={<PopoverContent filterRef={filterRef} filterKey={key} type={type} />}
