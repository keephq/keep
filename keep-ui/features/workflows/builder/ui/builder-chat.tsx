  ...
  useCopilotReadable(
    {
      ...
      convert: (description, value) => {
        return value
          ?.map((s: V2Step) => {
            const { componentType, name, type, properties } = s;
            if (componentType === "trigger") {
              return `componentType: ${componentType}, name: ${name}, type: ${type}, properties: ${JSON.stringify(properties, null, 2)}`;
            }
