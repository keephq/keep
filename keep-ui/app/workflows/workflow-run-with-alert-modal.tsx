...
export default function AlertTriggerModal({ isOpen, onClose, onSubmit, staticFields = [], dependencies = [] }: AlertTriggerModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const buildNestedObject = (acc: Record<string, any>, key: string, value: string) => {
    ...
    <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto" open={isOpen} onClose={onClose}>
      <div className="flex items-center justify-center min-h-screen">
        ...
        <div className="bg-white p-4 rounded max-w-lg max-h-fit mx-auto z-20">
          <Dialog.Title>Build Alert Payload</Dialog.Title>
          <form onSubmit={handleSubmit}>
            ...
            <Card className="mb-4">
              <Text className="mb-2">Fields Needed for Workflow (used in the workflow)</Text>
              {Array.isArray(dependencies) && dependencies.map((dependencyName, index) => (
                <div key={dependencyName} className="flex gap-2 mb-2">
                  <TextInput placeholder={dependencyName} value={dependencyName} disabled />
                  <TextInput
                    className={dependenciesErrors[index] ? 'border-red-500' : ''}
                    placeholder="value"
                    value={dependencyValues[dependencyName] || ""}
                    onChange={(e) => handleDependencyChange(dependencyName, e.target.value)}
                  />
                </div>
              ))}
            </Card>
            ...
            <Card>
              {dynamicFields.map((field, index) => (
                <div key={index} className={`flex items-center gap-2 mb-2 ${fieldErrors[index] ? 'border-2 border-red-500 p-2' : ''}`}>
                  <TextInput
                    placeholder="Key"
                    value={field.key}
              ...
              <div className="flex justify-center">
                <Button  color="orange"  onClick={handleAddField}>Add another field</Button>
              </div>
            </Card>

            <div className="mt-4 flex gap-2">
                <Button color="orange" type="submit">Run workflow</Button>
