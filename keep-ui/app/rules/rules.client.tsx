...
const CustomCombinatorSelector = (props: any) => {
  ...
  return <Title>Alert Group {path[0] + 1}</Title>;
};
...
const CustomAddRuleAction = (props: any) => {
  ...
  const availableFields = props.schema.fields.filter(
    (fld: any) => !props.rules.some((rule: any) => rule.field === fld.name)
  );
  ...
  return (
    <div className="relative inline-block">
      <Button
        ...
        className="text-white bg-orange-500 hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Add Condition
      </Button>
      <Icon
...
export default function Page() {
  ...
  const valueEditor = useMemo(() => {
    const Component = (props: any) => (
      <CustomValueEditor {...props} validationErrors={validationErrors} />
    );
  ...
  const CustomFieldSelector = (props: any) => {
    const { options, value, handleOnChange, path, currentQuery, setFields } =
      props;
    let groupIndex = 0;
    let ruleIndex = 0;
    ...
    let isRuleNew =
      ruleIndex === currentGroup.rules.length - 1 &&
      currentGroup.rules[ruleIndex].value === "";
    let currentRule = currentGroup.rules[ruleIndex];
    // Get other rules in the same group, excluding the current rule if it's not new
    let otherRules = currentGroup
      ? currentGroup.rules.filter(
          (index: any) => isRuleNew || index !== ruleIndex
        )
      : [];
    ...
    const filteredOptions = options.filter(
      (option: unknown) =>
        !otherRules.some(
          (rule: unknown) =>
            typeof rule === "object" &&
            (rule as any).field === (option as any).name
        )
    );
    ...
    if (!field) {
      const newField = {
        name: currentRule.field,
        label: currentRule.field,
    ...
    return (
      <CreatableSelect
        ...
        value={reactSelectOptions.find(
          (option: any) => option.value === currentRule.field
        )}
        onChange={(selectedOption) => handleOnChange(selectedOption.value)}
        options={reactSelectOptions}
        styles={{
  ...
  const addRule = (props: any) => {
    ...
    setQuery(
      add(
        query,
        { field: availableFields[0].name, operator: "=", value: "" },
        props.path
      )
    );
  };
  ...
  const saveRule = () => {
    ...
    const timeframeInSeconds =
      formData.timeframeUnit === "Seconds"
        ? formData.timeframe
        : formData.timeframeUnit === "Minutes"
        ? formData.timeframe * 60
        : formData.timeframeUnit === "Hours"
        ? formData.timeframe * 3600
        : formData.timeframe * 86400;
    fetch(`${apiUrl}/rules`, {
      method: "POST",
      headers: {
  ...
  const isTimeframeNumeric =
    !isNaN(parseFloat(formData.timeframe.toString())) &&
    isFinite(formData.timeframe);

  const getTimeframeErrorMessage = () => {
    if (
      formData.timeframe.toString() === "" ||
      formData.timeframe === undefined
    ) {
        ...
        <Card style={{ width: "50%" }}>
          ...
          <QueryBuilder
            ...
            controlElements={{
              ...
              addRuleAction: (props) => (
                <CustomAddRuleAction {...props} addRule={addRule} />
              ),
            ...
            controlClassnames={{
              queryBuilder:
                "queryBuilder-branches bg-orange-300 dark:bg-orange-950 !important rounded-lg shadow-xl",
              ruleGroup:
                "rounded-lg bg-orange-300 dark:bg-orange-950 bg-opacity-10 mt-4  !important",
          ...
          <div className="text-right">
            {!editMode && (
              <Button
                tooltip={canCreateRule() ? "" : "At least one rule is required"}
                className="mt-2"
                color="orange"
                  ...
                  {rules && rules.length > 0 ? (
                    ...
                    <>
                      ...
                      <TableRow>
                        <TableCell colSpan={6}>
                          <div>
                            <Subtitle className="whitespace-normal break-words">
